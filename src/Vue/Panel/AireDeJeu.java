...
public class AireDeJeu extends JPanel {
		...
		for (a = 0; a < this.niveau.getLongueur(); a++) {
			for (b = 0; b < this.niveau.getHauteur(); b++) {
				...
				case "Modele.Cases.Boue":
					g.drawImage(boue.getAnimation().getSprite(), a
							* Variables.TAILLE_CASE, b * Variables.TAILLE_CASE,
							null);
					break;
				case "Modele.Cases.MurNormal":
					g.drawImage(murNorm.getAnimation().getSprite(), a
							* Variables.TAILLE_CASE, b * Variables.TAILLE_CASE,
							null);
					break;
				case "Modele.Cases.MurIndestructible":
					g.drawImage(murInd.getAnimation().getSprite(), a
							* Variables.TAILLE_CASE, b * Variables.TAILLE_CASE,
							null);
					break;
				case "Modele.Cases.Sortie":
					g.drawImage(this.niveau.getSortie().getAnimation()
							.getSprite(), a * Variables.TAILLE_CASE, b
							* Variables.TAILLE_CASE, null);
					break;
				case "Modele.Cases.Personnage":
					g.drawImage(this.niveau.getPerso().getAnimation()
							.getSprite(), a * Variables.TAILLE_CASE, b
							* Variables.TAILLE_CASE, null);
					break;
				}
	...
	public void toucheDroite() {
		...
		niveau.getPerso().setAnimation(
				Personnage.getAnimations().get(Etats.MarcheDroite));
	...
	public void toucheGauche() {
		...
		if (!keys.get(KeyEvent.VK_RIGHT)) {
			...
			niveau.getPerso().setAnimation(
					Personnage.getAnimations().get(Etats.MarcheGauche));
			niveau.getPerso().getAnimation().start();
			repaint();
		}
	}
	...
	public void toucheHaut() {
		...
		if (!keys.get(KeyEvent.VK_RIGHT) && !keys.get(KeyEvent.VK_LEFT)) {
			...
			niveau.getPerso().setAnimation(
					Personnage.getAnimations().get(Etats.MarcheHaut));
			niveau.getPerso().getAnimation().start();
			repaint();
		}
	}
	...
	public void toucheBas() {
		...
		if (!keys.get(KeyEvent.VK_RIGHT) && !keys.get(KeyEvent.VK_LEFT)
			...
			niveau.getPerso().setAnimation(
					Personnage.getAnimations().get(Etats.MarcheBas));
			niveau.getPerso().getAnimation().start();
			repaint();
		}
	}
		...
		if (!keys.containsValue(true)) {
			...
			case KeyEvent.VK_RIGHT:
				if (keys.get(KeyEvent.VK_LEFT)) {
					...
					niveau.getPerso().setAnimation(
							Personnage.getAnimations().get(Etats.StopDroite));
			...
			case KeyEvent.VK_LEFT:
				if (keys.get(KeyEvent.VK_RIGHT)) {
					...
					niveau.getPerso().setAnimation(
							Personnage.getAnimations().get(Etats.StopGauche));
					niveau.getPerso().getAnimation().stop();
					niveau.getPerso().getAnimation().reset();
				}
				repaint();
			...
			case KeyEvent.VK_UP:
				if (keys.get(KeyEvent.VK_RIGHT)) {
					...
					niveau.getPerso().setAnimation(
							Personnage.getAnimations().get(Etats.StopHaut));
					niveau.getPerso().getAnimation().stop();
					niveau.getPerso().getAnimation().reset();
				}
				repaint();
			...
			case KeyEvent.VK_DOWN:
				if (keys.get(KeyEvent.VK_RIGHT)) {
					...
					niveau.getPerso().setAnimation(
							Personnage.getAnimations().get(Etats.StopBas));
					niveau.getPerso().getAnimation().stop();
					niveau.getPerso().getAnimation().reset();
				}
				repaint();
	...
	private void drawPerso(Graphics g) {
		g.drawImage(niveau.getPerso().getAnimation().getSprite(), this.niveau
				.getPerso().getPos_x(), this.niveau.getPerso().getPos_y(), null);
		Toolkit.getDefaultToolkit().sync();
	}
	...
	private void mouvement(int dir) {
		...
		case KeyEvent.VK_LEFT:
			...
			break;
		case KeyEvent.VK_UP:
			if (!(this.niveau.getPerso().getPos_y() * Variables.TAILLE_CASE <= 0))
				this.niveau.getPerso().setDeplace(Directions.Haut);
			break;
		case KeyEvent.VK_DOWN:
			if (!(this.niveau.getPerso().getPos_y() * Variables.TAILLE_CASE >= (this.niveau
					.getHauteur() * Variables.TAILLE_CASE)
					- (Variables.PAS_MVT)))
	...
	public void update() {
		if ((boolean) keys.get(KeyEvent.VK_RIGHT)) {
			...
			if (compteurIdle > idleDelay) {
				...
				niveau.getPerso().setAnimation(
						Personnage.getAnimations().get(Etats.StopIdle));
				niveau.getPerso().getAnimation().start();
			}
		}
		repaint();
